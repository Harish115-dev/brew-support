...
export const PaymentPage = ({ username }) => {
    ...
    useEffect(() => {
        if (serchParamscontext.get("paymentdone") === "true" && session) {
                ...
                <div className="bg-white border border-[rgba(45,26,14,0.08)] rounded-2xl p-4 sm:p-6 lg:p-8 shadow-sm h-full">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-6">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-xs text-[#b8a090] font-sans">
                                supporters
                            </p>
                        </div>
                    </div>
                ...
                <div className="lg:sticky lg:top-24 lg:self-start">
                    ...
                    <div className="bg-white border border-[rgba(45,26,14,0.08)] rounded-2xl p-4 sm:p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-4">
                            <button onClick={() => pay(50)} disabled={!paymentform.name.trim() || !paymentform.message.trim()} className="py-2 rounded-xl border hover:bg-[#f5ede0] disabled:bg-gray-200 disabled:cursor-not-allowed disabled:text-gray-500">
                                ₹50
                            </button>

                            <button onClick={() => pay(100)} disabled={!paymentform.name.trim() || !paymentform.message.trim()} className="py-2 rounded-xl border hover:bg-[#f5ede0] disabled:bg-gray-200 disabled:cursor-not-allowed disabled:text-gray-500">
                                100
                            </button>

                            <button onClick={() => pay(200)} disabled={!paymentform.name.trim() || !paymentform.message.trim()} className="py-2 rounded-xl border hover:bg-[#f5ede0] disabled:bg-gray-200 disabled:cursor-not-allowed disabled:text-gray-500">
                                200
                            </button>
                        </div>

                        <button onClick={() => pay(paymentform.amount)} disabled={!isFormValid} className="w-full py-3 rounded-xl bg-[#b8832a] text-white font-semibold hover:bg-[#9c6f24] transition-all disabled:bg-gray-400 disabled:cursor-not-allowed">
                            Support Now
                        </button>
